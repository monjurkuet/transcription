...
class MistralProvider(TranscriptionProvider):
    ...
    def transcribe(
        ...
        if not key_state:
            raise RuntimeError("No Mistral keys are currently available")
        ...
        try:
            ...
            message = f"Mistral request failed: {exc}"
            self.key_pool.mark_error(key_state.key_id, message)
            raise coerce_provider_error(503, message) from exc

        if response.status_code != 200:
            message = f"Mistral transcription failed ({response.status_code}): {response.text}"
